...
export default async function SharedRendaVariavelPage({
  params,
}: {
  ...
  const tickers = Array.from(
    new Set(transactions.map((t) => t.assets?.ticker).filter((t): t is string => !!t)),
  );
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">Renda Variável</h1>
      ...
      {openPositions.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Quanto o dono tem de cada ativo hoje, pela cotação atual
            </CardDescription>
          </CardHeader>
          <CardContent>
                    ...
                    return (
                      <TableRow key={p.asset_id}>
                        <TableCell className="font-medium">
                          <Link
                            href={`/compartilhado-comigo/${ownerId}/renda-variavel/${p.asset_id}`}
                  ...
                  {transactions.map((tx) => {
                    ...
                    return (
                      <TableRow key={tx.id}>
                        <TableCell className="font-medium">
                          <Link
                            href={`/compartilhado-comigo/${ownerId}/renda-variavel/${tx.asset_id}`}
                            className="underline underline-offset-4"
                          >
                            {ticker ?? "-"}
                          </Link>
